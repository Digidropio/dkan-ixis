 ...
 */
function insight_form_system_theme_settings_alter(&$form, &$form_state) {
  global $theme;
  // Ensure this include file is loaded when the form is rebuilt from the cache.
  $form_state['build_info']['files']['form'] = drupal_get_path('theme', 'insight') . '/theme-settings.php';
  ...
  $copyright = theme_get_setting('copyright', 'insight');
  ...
  // Log In Display.
  $display_login_menu = (theme_get_setting('display_login_menu', 'insight') === NULL) ? 1 : theme_get_setting('display_login_menu', 'insight');
  ...
  // Upload field.
  $hero = theme_get_setting('hero_file', 'insight');
  ...
  $form['hero']['background_option'] = array(
    ...
    '#required' => FALSE,
    '#default_value' => theme_get_setting('background_option', 'insight'),
    '#element_validate' => array('_insight_background_option_setting'),
  );
 ...
 */
function _insight_background_option_setting($element, &$form, &$form_state) {
 ...
 */
function insight_hero_system_theme_settings_form_submit(&$form, &$form_state) {
  if ($form_state['values']['hero_file']) {
    $fid = $form_state['values']['hero_file'];
    insight_file_set_permanent($fid);
  }
 ...
 */
function insight_file_set_permanent($fid) {
  $file = file_load($fid);
  $file->status = FILE_STATUS_PERMANENT;
  file_save($file);
  file_usage_add($file, 'theme', 'file', $fid);
  insight_file_insert($file);
}

/**
* Implements hook_file_insert().
*/
function insight_file_insert($file) {
 ...
 * @remarks - This is not generated by radix automatically.
 *
 * @see radix_site_information_theme_settings_form_submit()
 */
function insight_site_information_theme_settings_form_submit(&$form, &$form_state) {
